...
set LIBNAME=%3 frocksdbplugins
if "%LIBNAME%"=="" (set LIBNAME=frocksdbplugins)

LIBFULLNAME=%LIBNAME%jni-win64
...
cmake -G "Visual Studio 15 Win64" .. -DROCKSDB_PATH=%ROCKSDB_SOURCE_PATH% -DLIBNAME=%LIBFULLNAME%

"%MSBUILD%" %LIBNAME%.sln /p:Configuration=Release /m

cd %CWD%

set LIB=lib%LIBFULLNAME%.dll
